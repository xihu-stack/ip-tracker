...
const pageList = ref([
  { name: '仪表盘', tag: '', desc: '查看设备总数、在线数、离线数。下方中国地图展示设备分布位置（橙色光点），鼠标悬停可查看城市和设备数量。在线和离线设备列表支持滚动查看。' },
  { name: '员工列表', tag: 'success', desc: '查看所有已部署的员工电脑，显示员工姓名、主机名、驻地、最新 IP、所在城市、最后上报时间和在线状态。支持按姓名或主机名搜索。可编辑员工姓名和驻地（常驻办公城市）、查看历史记录，也可删除已离职或已卸载的员工设备。' },
  { name: 'IP 历史', tag: 'warning', desc: '选择某个员工，按日期范围查询该员工的所有 IP 上报记录，包括每次上报的时间和解析出的城市。' },
  { name: '位置动态', tag: 'danger', desc: '重点看哪些人员不在驻地办公：顶部统计异地/在驻地/位置未知/未设驻地人数；下方列出当前异地人员（驻地、当前城市、外出开始时间和已外出时长，超 3 天标红）以及最近 7 天的位置变更记录（从哪到哪）。前提：在员工列表中为员工设置驻地（可用"填入当前城市"一键设置）。' },
])
