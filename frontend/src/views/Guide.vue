  ...
  <div>
    ...
    <el-card style="margin-bottom: 16px">
      ...
      <el-alert type="info" :closable="false" style="margin-top: 12px" show-icon>
        <template #title>
          设备总数 = 在线 + 离线，互为补集。判定依据是员工电脑的上报时间戳，不是主动探测。电脑关机/断网后最多 10 分钟页面会显示离线。
        </template>
      </el-alert>
    </el-card>
    ...
    <el-card>
      ...
      <p style="line-height: 1.8; margin: 0; color: #606266">
        同一员工、同一 IP 地址，<b>5 分钟内</b>不会重复记录。例如员工电脑每 10 分钟上报一次，
...
const pageList = ref([
  { name: '仪表盘', tag: '', desc: '查看设备总数、在线数、离线数。下方中国地图展示设备分布位置（黄色圆点），鼠标悬停可查看城市和设备数量。' },
  { name: '员工列表', tag: 'success', desc: '查看所有已部署的员工电脑，显示员工姓名、主机名、最新 IP、所在城市、最后上报时间和在线状态。支持按姓名或主机名搜索，点击"编辑"可填写员工姓名，点击"查看历史"可跳转到该员工的 IP 变更记录。' },
  { name: 'IP 历史', tag: 'warning', desc: '选择某个员工，按日期范围查询该员工的所有 IP 上报记录，包括每次上报的时间和解析出的城市。' },
])

const statusList = ref([
  { status: '在线', tag: 'success', condition: '最近 10 分钟内有上报记录', meaning: '电脑开机且网络正常，脚本正常运行' },
  { status: '离线', tag: 'warning', condition: '超过 10 分钟没有上报记录', meaning: '电脑关机、断网、或脚本异常' },
])
